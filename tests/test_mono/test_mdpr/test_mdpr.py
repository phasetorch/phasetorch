...
def test_tie_tie(params):
    ...
    func = inspect.getframeinfo(inspect.currentframe()).function
    rads, pdelta_gt, pbeta_gt, pmask_gt = sim_rads(forw_model="TIE", **params)
...
def eval_nlpr(rads, pdelta_gt, pmask_gt, num_z, num_x, pix_width, energy, prop_dists, processes, device, save_dir, tag, **kwargs):
    ...
    solver_kwargs = {'solver': 'non-native', 'history_size': 64, 'max_iter': 1000, 'line_search_fn': 'Wolfe', 'rec_thresh': 0.5, 'rec_convg': True, 'cost_thresh': 1.0, 'cost_convg': False, 'chk_iters': 5}   
