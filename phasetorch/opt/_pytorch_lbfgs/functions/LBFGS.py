...
def polyinterp(points, x_min_bound=None, x_max_bound=None, plot=False):
    ...
    if no_points == 2 and order == 2 and plot is False:
        ...
        if points[0, 0] == 0:
            ...
            a = -(points[0, 1] - points[1, 1] - points[0, 2] * (points[0,
                  0] - points[1, 0])) / (points[0, 0] - points[1, 0]) ** 2
        ...
        for i in range(no_points):
            if not np.isnan(points[i, 2]):
                ...
                for j in range(order):
                    constraint[0, j] = (order - j) * \
                        points[i, 0] ** (order - j - 1)
                A = np.append(A, constraint, 0)
...
class LBFGS(Optimizer):
    ...
    def __init__(self, params, lr=1., history_size=10, line_search='Wolfe',
                 dtype=torch.float, debug=False, device="cuda"):
        ...
        defaults = dict(lr=lr, history_size=history_size,
                        line_search=line_search, dtype=dtype, debug=debug, device=device)
